...
<div class="contents">
	...
	<div id="list_contain" class="ui-widget">
		...
		<div id="paging" style="text-align:center; font-size:20px; padding:1em">
   		<span>
   			<c:if test="${blockNo > 1}">
   				<span><a href="pjt.do?cmd=staff_pjt_list&pageNo=${prevPageNo}">&laquo;이전 페이지</a></span>
   			</c:if>
       		<c:choose>
       		<c:when test="${blockNo < totBlock }">
        		<c:forEach var="i" begin="${firstPageNoInBlock}" end="${lastPageNoInBlock}" step="1">
            		<c:choose>
                		...
                		<c:otherwise><a href="pjt.do?cmd=staff_pjt_list&pageNo=${i}">${i}</a></c:otherwise>
            		</c:choose>
        		</c:forEach>
        	</c:when>
        	<c:otherwise>
       		...
       		<c:if test="${blockNo < totBlock}">
   				<span><a href="pjt.do?cmd=staff_pjt_list&pageNo=${nextPageNo}">다음 페이지&raquo;</a></span>
   			</c:if>
   		</span>
		</div>
	</div>
